...
int main(int argc, char *argv[]) {
  try {
    ...
    po.Register("sort-order", &sort_order, "If true, sort the order "
                "of the output by mat.NumRows()");
    ...
    if (sort_order)
      std::sort(egs.begin(), egs.end(), SortNnetCtcExampleByNumFrames);

    NnetCtcExampleWriter writer(examples_wspecifier);
