...
int main(int argc, char *argv[]) {
  ...
  try {
    const char *usage =
      "Relabel neural network egs with the read pdf-id/ctc alignments, "
      "zero-based..\n"
      "Usage: nnet-ctc-relabel-egs [options] <pdf-aligment-rspecifier> "
      "<egs_rspecifier1> ... <egs_rspecifierN> "
      "<egs_wspecifier1> ... <egs_wspecifierN>\n"
      "e.g.: \n"
      " nnet-ctc-relabel-egs ark:1.ali egs_in/egs.1.ark egs_in/egs.2.ark "
    ...
    for (int32 i = 0; i < num_archives; i++) {
      ...
      SequentialNnetCtcExampleReader egs_reader(egs_rspecifier);
      NnetCtcExampleWriter egs_writer(egs_wspecifier);

      for (; !egs_reader.Done(); egs_reader.Next(), num_frames_egs++) {

        std::string key(egs_reader.Key());

        std::string utt_id = key;

        NnetCtcExample eg(egs_reader.Value());
