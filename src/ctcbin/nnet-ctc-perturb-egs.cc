...
int main(int argc, char *argv[]) {
  try {
    ...
    const char *usage =
      ...
      "\n"
      "Usage:  nnet-ctc-perturb-egs [options] <cholesky> <egs-rspecifier> <egs-wspecifier>\n"
      "\n"
      "nnet-ctc-perturb-egs --noise-factor=0.2 exp/nnet5/cholesky.tpmat ark:- ark:-\n";
    ...
    for (; !example_reader.Done(); example_reader.Next(), num_done++) {
      ...
      NnetCtcExample eg = example_reader.Value();
