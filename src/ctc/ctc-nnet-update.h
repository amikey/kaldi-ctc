...
#ifndef KALDI_CTC_CTC_NNET_UPDATE_H_
#define KALDI_CTC_CTC_NNET_UPDATE_H_
...
// This class NnetCtcUpdater contains functions for updating the neural net or
// computing its gradient, given a set of NnetCtcExamples. We
...
class NnetCtcUpdater {
 public:
  ...
  NnetCtcUpdater(const Nnet &nnet,
                 Nnet *nnet_to_update);
  ...
  double ComputeForMinibatch(const std::vector<NnetCtcExample> &data,
                             double *tot_accuracy);
 ...
 protected:
  ...
  void FormatInput(const std::vector<NnetCtcExample> &data);

  /// Computes objective function and derivative at output layer, but does not
  /// do the backprop [for that, see Backprop()].  Returns wether error occurs
  ...
  bool ComputeObjfAndDeriv(const std::vector<NnetCtcExample> &data,
 ...
 private:
  ...
  double ComputeTotAccuracy(const std::vector<NnetCtcExample> &data,
                            BaseFloat *tot_weight = NULL) const;
...
/// a class NnetCtcUpdater that's defined in nnet-update.cc, but we
