...
#
# This script differs from get_egs.sh in that it dumps egs with several utts
# of labels, controlled by the utts_per_eg config variable (default: 8).  This
# takes many times less disk space because typically we have 4 to 7 utts of
...
# $dir/info/utts_per_eg, which contains the number of utts of labels per eg
...
utts_per_eg=5000     # number of utts of labels per example.  more->less disk space and
                  # less time preparing egs, but more I/O during training.
                  # note: the script may reduce this if reduce_utts_per_eg is true.
left_context=0    # amount of left-context per eg
right_context=0   # amount of right-context per eg
delta_order=      # delta feature order

reduce_utts_per_eg=true  # If true, this script may reduce the utts_per_eg
                           # if there is only one archive and even with the
                           # reduced utts_pe_eg, the number of
                           # utts_per_eg that would result is less than or
                           # equal to the user-specified value.
num_utts_subset=300     # number of utterances in validation and training
                        # subsets used for shrinkage and diagnostics.
num_valid_utts_combine=0 # #valid utts for combination weights at the very end.
num_train_utts_combine=10000 # # train utts for the above.
num_utts_diagnostic=4000 # number of utts for "compute_prob" jobs
...
stage=0
io_opts="-tc 10" # for jobs with a lot of I/O, limits the number running at one time.
random_copy=false
...
if [ $# != 3 ]; then
  ...
  echo "  --utts-per-eg <utts;8>                           # number of utts per eg on disk"
  echo "  --left-context <width;4>                         # Number of utts on left side to append for feature input"
  echo "  --right-context <width;4>                        # Number of utts on right side to append for feature input"
  echo "  --num-utts-diagnostic <#utts;4000>               # Number of utts used in computing (train,valid) diagnostics"
  echo "  --num-valid-utts-combine <#utts;10000>           # Number of utts used in getting combination weights at the"
  echo "                                                   # very end."
...
data=$1
alidir=$2 # key -> chars' id sort by length
dir=$3
...
for f in $data/feats.scp $alidir/ali.1.gz $extra_files; do
...
if [[ -f $data/utt2uniq && ! -f $dir/valid_uttlist ]]; then
...
# if [ $stage -le -1 ] && [ ! -f $data/utt2rows ]; then
#   # compute length, sort the length
#   $cmd JOB=1:$nj $dir/log/utt2rows.JOB.log \
#     feat-to-len scp:$sdata/JOB/feats.scp ark,t:- | sort -V -k2 >$sdata/JOB/utt2rows || exit 1;
#   cat $sdata/*/utt2rows | sort -V -k2 >$data/utt2rows || exit 1;
#   awk 'FNR==NR{U[$1]=1;} FNR<NR{if($1 in U) print $0;}' $dir/valid_uttlist $data/utt2rows > $dir/valid_utt2rows || exit 1;
#   awk 'FNR==NR{U[$1]=1;} FNR<NR{if($1 in U) print $0;}' $dir/train_subset_uttlist $data/utt2rows > $dir/train_subset_utt2rows || exit 1;
#   raw) feats="ark:utils/filter_scp.pl --exclude $dir/valid_uttlist $sdata/JOB/feats.scp | awk 'FNR==NR{F[$1]=$0;} FNR<NR{if($1 in F){print F[$1]}}' - $sdata/JOB/utt2rows | apply-cmvn $cmvn_opts --utt2spk=ark:$sdata/JOB/utt2spk scp:$sdata/JOB/cmvn.scp scp:- ark:- |"
#     valid_feats="ark:utils/filter_scp.pl $dir/valid_uttlist $data/feats.scp | awk 'FNR==NR{F[$1]=$0;} FNR<NR{if($1 in F){print F[$1]}}' - $dir/valid_utt2rows | apply-cmvn $cmvn_opts --utt2spk=ark:$data/utt2spk scp:$data/cmvn.scp scp:- ark:- |"
#     train_subset_feats="ark:utils/filter_scp.pl $dir/train_subset_uttlist $data/feats.scp | awk 'FNR==NR{F[$1]=$0;} FNR<NR{if($1 in F){print F[$1]}}' - $dir/train_subset_utt2rows | apply-cmvn $cmvn_opts --utt2spk=ark:$data/utt2spk scp:$data/cmvn.scp scp:- ark:- |"
...
if [ $stage -le 0 ]; then
  echo "$0: working out number of utts of training data"
  num_utts=$(cat $data/utt2spk | wc -l)
  echo $num_utts > $dir/info/num_utts
else
  num_utts=`cat $dir/info/num_utts` || exit 1;
fi

# the + 1 is to round up, not down... we assume it doesn't divide exactly.
num_archives=$[$num_utts/($utts_per_eg)+1]
# (for small data)- while reduce_utts_per_eg == true and the number of
# archives is 1 and would still be 1 if we reduced utts_per_eg by 1, reduce it
# by 1.
reduced=false
while $reduce_utts_per_eg && [ $utts_per_eg -gt 1 ] && \
  [ $[$num_utts/$utts_per_eg] -eq 0 ]; do
  utts_per_eg=$num_utts
  num_archives=1
  reduced=true
done
$reduced && echo "$0: reduced utts_per_eg to $utts_per_eg because amount of data is small."

echo $num_archives >$dir/info/num_archives
echo $utts_per_eg >$dir/info/utts_per_eg

# Working out number of egs per archive
egs_per_archive=$utts_per_eg

echo $egs_per_archive > $dir/info/egs_per_archive

echo "$0: creating $num_archives archives, each with $egs_per_archive egs, with"
echo "$0:   $utts_per_eg labels per archives, and (left,right) context = ($left_context,$right_context)"
...
if [ $stage -le 2 ]; then
  ...
  $cmd $dir/log/create_valid_subset.log \
    nnet-ctc-get-egs $ivectors_opt $nnet_context_opts "$valid_feats" \
    "ark:gunzip -c $dir/ali_special.gz |" "ark:$dir/valid_all.egs" || touch $dir/.error &
  $cmd $dir/log/create_train_subset.log \
    nnet-ctc-get-egs $ivectors_opt $nnet_context_opts "$train_subset_feats" \
     "ark:gunzip -c $dir/ali_special.gz |" "ark:$dir/train_subset_all.egs" || touch $dir/.error &
  wait;
  [ -f $dir/.error ] && echo "Error detected while creating train/valid egs" && exit 1
  echo "... Getting subsets of validation examples for diagnostics and combination."
  $cmd $dir/log/create_valid_subset_combine.log \
    nnet-ctc-subset-egs --n=$num_valid_utts_combine ark:$dir/valid_all.egs \
        ark:$dir/valid_combine.egs || touch $dir/.error &
  $cmd $dir/log/create_valid_subset_diagnostic.log \
    nnet-ctc-subset-egs --n=$num_utts_diagnostic ark:$dir/valid_all.egs \
    ark:$dir/valid_diagnostic.egs || touch $dir/.error &

  $cmd $dir/log/create_train_subset_combine.log \
    nnet-ctc-subset-egs --n=$num_train_utts_combine ark:$dir/train_subset_all.egs \
    ark:$dir/train_combine.egs || touch $dir/.error &
  $cmd $dir/log/create_train_subset_diagnostic.log \
    nnet-ctc-subset-egs --n=$num_utts_diagnostic ark:$dir/train_subset_all.egs \
...
if [ $stage -le 3 ]; then
  ...
  $cmd $io_opts JOB=1:$nj $dir/log/get_egs.JOB.log \
    nnet-ctc-get-egs $ivectors_opt $nnet_context_opts "$feats" \
    "ark:gunzip -c $alidir/ali.*.gz |" ark:- \| \
    nnet-ctc-copy-egs ark:- $egs_list || exit 1;
fi

if [ $stage -le 4 ]; then
  echo "$0: recombining and sorting order of archives on disk"
  # combine all the "egs_orig.JOB.*.scp" (over the $nj splits of the data) and
  # sort the order, writing to the egs.JOB.ark
  ...
  $cmd --max-jobs-run $nj $io_opts $extra_opts JOB=1:$num_archives $dir/log/sort.JOB.log \
    nnet-ctc-sort-egs "ark:cat $egs_list|" ark:$dir/egs.JOB.ark  || exit 1;
fi
