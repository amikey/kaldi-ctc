...
if [ $stage -le 3 ]; then
  awk '{printf $1; for (i=2; i<=NF; ++i) {gsub(/[0-9]/, "", $i); printf(" %s", $i);} print "";}' $lexicon_raw_nosil >$lexicon_raw_nosil.clean
  ...
  awk '{for (i=2; i<=NF; ++i) { print $i; gsub(/[0-9]/, "", $i); print $i}}' $lexicon_raw_nosil.clean |\
    sort -u |\
    perl -e 'while(<>){
...
if [ $stage -le 4 ]; then
  ...
  cat - $lexicon_raw_nosil.clean | sort | uniq >$dst_dir/lexicon.txt
